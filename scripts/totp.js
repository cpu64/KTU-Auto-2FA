...
function getTOTP(secret, expiry, length, foo) {
    crypto.subtle.importKey("raw", Base32ToUint8Array(secret), { name: "HMAC", hash: "SHA-1" }, false, ["sign"]).then(function (key) {
        crypto.subtle.sign({ name: "HMAC", hash: "SHA-1" }, key, HexToUint8Array("00000000" + Int32ToHex((Date.now() / 1000) / expiry))).then(function (temp) {
            let hex = Uint8ArrayToHex(new Uint8Array(temp));
            let start = parseInt(hex.substr(-1), 16);
            let num = parseInt(hex.substr(start * 2, 8), 16) & 0x7FFFFFFF;
            let code = num % Math.pow(10, length);
            foo(("0000000000" + code).substr(-length));
        });
    });
};
...
function Base32ToUint8Array(base32) {
    ...
    for (let i = 0; i < binary.length; i += 8) {
        array.push(parseInt(binary.substr(i, 8), 2));
    }
...
function Int32ToHex(int32) {
    return ("00000000" + Math.floor(Math.abs(int32)).toString(16)).substr(-8);
};
